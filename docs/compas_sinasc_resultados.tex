...
O resultado mais expressivo da auditoria de \textit{gerrymandering} no SINASC é o cruzamento entre faixa etária materna e escolaridade: enquanto os gaps marginais de cada variável isoladamente são pequenos ($2,33\%$ e $4,96\%$), o gap real observado ao cruzá-las atinge $7,56\%$ -- acima do gap esperado, revelando um viés oculto de $+2,60$ pontos percentuais. Ainda que a magnitude absoluta seja substancialmente menor do que a observada no Adult e no COMPAS, o padrão estrutural se repete: a análise unidimensional, isoladamente, não seria capaz de identificar essa disparidade adicional que emerge apenas no cruzamento entre o proxy socioeconômico e o atributo sensível.
...
Os valores de CDDL do SINASC são substancialmente menores, em magnitude absoluta, do que os observados no Adult (máximo de $0,0353$ contra $0,3106$), consistente com a menor disparidade marginal já discutida. Ainda assim, o padrão estrutural se repete: a faixa etária materna madura ($>$34 anos) concentra as maiores disparidades condicionais, independentemente da variável de estratificação utilizada -- sugerindo que, mesmo em um contexto de disparidades marginais pequenas, a combinação com outros atributos preserva uma estrutura de desvantagem relativa consistente. Cabe ressaltar, contudo, que o desbalanceamento extremo de classes do SINASC ($9,4\%$ de casos desfavoráveis) comprime mecanicamente a amplitude em que essas disparidades absolutas podem se manifestar: como a taxa de desfecho favorável da grande maioria dos subgrupos está concentrada em uma faixa estreita, próxima do teto de $90\%$, o espaço numérico disponível para qualquer gap absoluto -- seja em CI, DI, KL, KS ou CDDL -- é substancialmente menor do que em bases mais balanceadas como o Adult, independentemente da severidade real da disparidade subjacente. Essa característica deve ser levada em conta na interpretação das magnitudes reportadas nesta subseção, e antecipa o problema mais amplo de desbalanceamento discutido a seguir.
...
O \textit{baseline} adotado como referência para esta análise -- Gradient Boosting otimizado por AUROC -- atingiu acurácia de $90,57\%$, AUROC de $0,533$ e AUPRC de $0,914$. Cabe registrar que a combinação Gradient Boosting/Random Forest otimizada por AUPRC (\textit{average\_precision}) não produziu resultados válidos em escala completa para o SINASC -- provavelmente por esgotamento de tempo ou memória durante a busca de hiperparâmetros nas $2,47$ milhões de instâncias, já que testes em subamostras de até $300$ mil linhas com a mesma configuração não reproduziram a falha --, e foi por isso excluída desta comparação. Entre as demais configurações válidas, Gradient Boosting e Random Forest exibem desempenho estatisticamente indistinguível entre si (acurácia idêntica de $90,57\%$ em todas elas; AUROC entre $0,530$ e $0,533$; AUPRC entre $0,913$ e $0,914$), de modo que a escolha do Gradient Boosting como referência é apenas uma convenção de apresentação -- consistente com os demais conjuntos de dados deste capítulo -- e não reflete uma vantagem real de um modelo sobre o outro. Esse resultado, à primeira vista favorável, esconde um problema crítico: a acurácia de $90,57\%$ \textbf{coincide exatamente} com a proporção da classe majoritária no conjunto de dados ($90,6\%$), e o AUROC correspondente é de apenas $0,533$ -- marginalmente acima do nível do acaso ($0,5$). Isso indica que o modelo colapsou para prever sistematicamente a classe majoritária (``peso normal'') para toda e qualquer instância, independentemente dos atributos de entrada.

\begin{table}[H]
    \centering
    \caption{Métricas de equidade interseccional pós-treino por subgrupo (SINASC, GradientBoosting otimizado por AUROC, modelo de referência) -- amostra de subgrupos selecionados, média entre 3 folds externos.}
    \vspace{2mm}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{l|cccccc}
        \hline
